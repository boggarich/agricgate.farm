<footer>
    <div class="container-xxl">

        <div class='row row-cols-lg-2 g-4'>

            <div class='col-lg-3 company-info-desktop'>
                    <img src='/assets/img/agricgate-farm-logo-3.png' alt="footer-logo" />
                    <p>AgricGate EduTech Company LTD.
                    <br>
                    Registration number: C0062885456
                    <br>
                    G231 Tingatinga Street, Dansoman Keep fit.
                    <br>
                    </p>
                    <p>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-telephone me-3" viewBox="0 0 16 16">
                        <path d="M3.654 1.328a.678.678 0 0 0-1.015-.063L1.605 2.3c-.483.484-.661 1.169-.45 1.77a17.6 17.6 0 0 0 4.168 6.608 17.6 17.6 0 0 0 6.608 4.168c.601.211 1.286.033 1.77-.45l1.034-1.034a.678.678 0 0 0-.063-1.015l-2.307-1.794a.68.68 0 0 0-.58-.122l-2.19.547a1.75 1.75 0 0 1-1.657-.459L5.482 8.062a1.75 1.75 0 0 1-.46-1.657l.548-2.19a.68.68 0 0 0-.122-.58zM1.884.511a1.745 1.745 0 0 1 2.612.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.68.68 0 0 0 .178.643l2.457 2.457a.68.68 0 0 0 .644.178l2.189-.547a1.75 1.75 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.6 18.6 0 0 1-7.01-4.42 18.6 18.6 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877z"/>
                        </svg>
                         WhatsApp/Tel: 0303-956125
                    </p> 
                    <p>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-envelope me-3" viewBox="0 0 16 16">
                            <path d="M0 4a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2zm2-1a1 1 0 0 0-1 1v.217l7 4.2 7-4.2V4a1 1 0 0 0-1-1zm13 2.383-4.708 2.825L15 11.105zm-.034 6.876-5.64-3.471L8 9.583l-1.326-.795-5.64 3.47A1 1 0 0 0 2 13h12a1 1 0 0 0 .966-.741M1 11.105l4.708-2.897L1 5.383z"/>
                        </svg>
                    admin@agricgate.farm
                </p>
                    <div class='social-media-handles d-flex mb-5'>

                        <a href="https://www.linkedin.com/company/agricgatefarm/">

                            <svg width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg" class="linkedin-logo footer">
                                <path d="M72 12H18C14.685 12 12 14.685 12 18V72C12 75.315 14.685 78 18 78H72C75.315 78 78 75.315 78 72V18C78 14.685 75.315 12 72 12ZM32.862 66H24.012V37.524H32.862V66ZM28.347 33.453C25.494 33.453 23.187 31.14 23.187 28.293C23.187 25.446 25.497 23.136 28.347 23.136C31.191 23.136 33.504 25.449 33.504 28.293C33.504 31.14 31.191 33.453 28.347 33.453ZM66.012 66H57.168V52.152C57.168 48.849 57.108 44.601 52.569 44.601C47.964 44.601 47.256 48.198 47.256 51.912V66H38.412V37.524H46.902V41.415H47.022C48.204 39.177 51.09 36.816 55.395 36.816C64.356 36.816 66.012 42.714 66.012 50.382V66Z" fill="#7F7F7F"/>
                            </svg>

                        </a>

                        <a href="https://web.facebook.com/agricgate.farm/">
                            <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 56 56" fill="none">
                                <path d="M56.007 28.175C56.007 12.614 43.47 0 28.007 0C12.537 0.0035 0 12.614 0 28.1785C0 42.238 10.241 53.893 23.625 56.007V36.3195H16.52V28.1785H23.632V21.966C23.632 14.9065 27.8145 11.0075 34.209 11.0075C37.275 11.0075 40.4775 11.557 40.4775 11.557V18.487H36.946C33.4705 18.487 32.3855 20.6605 32.3855 22.89V28.175H40.1485L38.9095 36.316H32.382V56.0035C45.766 53.8895 56.007 42.2345 56.007 28.175Z" fill="#7F7F7F"/>
                            </svg>
                        </a>

                        <a href="https://twitter.com/Agricgatefarm">
                            <svg xmlns="http://www.w3.org/2000/svg" width="68" height="48" viewBox="0 0 68 48" fill="none">
                                <path d="M21.1092 47.5676C46.4688 47.5676 60.3414 29.2623 60.3414 13.4159C60.3414 12.9036 60.3414 12.3841 60.3162 11.8718C63.017 10.1686 65.348 8.05987 67.2 5.64448C64.6771 6.61353 62.0036 7.2522 59.2662 7.53976C62.1496 6.03831 64.3092 3.6759 65.3436 0.891636C62.6345 2.28977 59.6695 3.27125 56.5782 3.7931C54.5 1.86498 51.7499 0.587651 48.754 0.158979C45.758 -0.269692 42.6834 0.174215 40.0064 1.42194C37.3294 2.66966 35.1994 4.65157 33.9463 7.06067C32.6932 9.46978 32.3869 12.1716 33.075 14.7477C27.5929 14.5082 22.2298 13.2676 17.3336 11.1062C12.4373 8.94483 8.11727 5.911 4.6536 2.2015C2.89523 4.84716 2.35861 7.97663 3.15271 10.9544C3.94682 13.9323 6.01211 16.5352 8.9292 18.2346C6.74327 18.1699 4.60539 17.6585 2.688 16.7418V16.9064C2.69177 19.6779 3.79389 22.3633 5.80807 24.5086C7.82225 26.6539 10.6249 28.1276 13.7424 28.6806C12.5591 28.9646 11.3368 29.1061 10.1094 29.1014C9.24413 29.1036 8.3806 29.0338 7.5306 28.8928C8.4117 31.2786 10.1273 33.3645 12.4372 34.8585C14.7471 36.3524 17.5356 37.1795 20.412 37.224C15.5255 40.5675 9.48948 42.3808 3.276 42.372C2.18115 42.376 1.08706 42.321 0 42.2074C6.30636 45.7098 13.6302 47.5696 21.1092 47.5676Z" fill="#7F7F7F"/>
                            </svg>
                        </a>

                    </div>
            </div>
            <div class="col-lg-3 d-flex flex-column">
                <h5 class="">{{ __('About') }}</h5>
                <a href="{{ route('who-we-are') }}">About Us</a>
                <a href="{{ route('contact-us') }}">Contact Us</a>
                <a href="{{ route('contact-us') }}">Privacy Policy</a>
                <a href="{{ route('contact-us') }}">Terms of Service</a>

            </div>
            <div class="col-lg-3 d-flex flex-column">
                <h5 class="">{{ __('Helpful Links') }}</h5>
                <a href="{{ route('explore') }}">Explore</a>
                <a href="{{ route('donate.create-charge') }}">Donate</a>
                <a href="{{ route('blogs-page') }}">News</a>
            </div>
            <div class="col-lg-3 d-flex flex-column">
                <h5 class="">{{ __('Newsletter Signup') }}</h5>
                <a href="{{ route('explore') }}">Explore</a>
                <a href="{{ route('donate.create-charge') }}">Donate</a>
                <a href="{{ route('blogs-page') }}">News</a>
            </div>
            
            <div class='col-lg-4 mt-5'>

                <div class="company-info-mobile ">
                    <img src='/assets/img/agricgate-farm-logo-3.png' alt="footer-logo" />
                    <p>Trisolace Company Ltd.
                    <br>
                    G231 Tingatinga Street
                    <br>
                    Dansoman, Accra - Ghana
                    <br>
                    </p>
                    <div class='social-media-handles mb-5'>

                        <a href="https://www.linkedin.com/company/agricgatefarm/">

                            <svg width="90" height="90" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg" class="linkedin-logo footer">
                                <path d="M72 12H18C14.685 12 12 14.685 12 18V72C12 75.315 14.685 78 18 78H72C75.315 78 78 75.315 78 72V18C78 14.685 75.315 12 72 12ZM32.862 66H24.012V37.524H32.862V66ZM28.347 33.453C25.494 33.453 23.187 31.14 23.187 28.293C23.187 25.446 25.497 23.136 28.347 23.136C31.191 23.136 33.504 25.449 33.504 28.293C33.504 31.14 31.191 33.453 28.347 33.453ZM66.012 66H57.168V52.152C57.168 48.849 57.108 44.601 52.569 44.601C47.964 44.601 47.256 48.198 47.256 51.912V66H38.412V37.524H46.902V41.415H47.022C48.204 39.177 51.09 36.816 55.395 36.816C64.356 36.816 66.012 42.714 66.012 50.382V66Z" fill="#7F7F7F"/>
                            </svg>

                        </a>

                        <a href="https://web.facebook.com/agricgate.farm/">
                            <svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 56 56" fill="none">
                                <path d="M56.007 28.175C56.007 12.614 43.47 0 28.007 0C12.537 0.0035 0 12.614 0 28.1785C0 42.238 10.241 53.893 23.625 56.007V36.3195H16.52V28.1785H23.632V21.966C23.632 14.9065 27.8145 11.0075 34.209 11.0075C37.275 11.0075 40.4775 11.557 40.4775 11.557V18.487H36.946C33.4705 18.487 32.3855 20.6605 32.3855 22.89V28.175H40.1485L38.9095 36.316H32.382V56.0035C45.766 53.8895 56.007 42.2345 56.007 28.175Z" fill="#7F7F7F"/>
                            </svg>
                        </a>

                        <a href="https://twitter.com/Agricgatefarm">
                            <svg xmlns="http://www.w3.org/2000/svg" width="68" height="48" viewBox="0 0 68 48" fill="none">
                                <path d="M21.1092 47.5676C46.4688 47.5676 60.3414 29.2623 60.3414 13.4159C60.3414 12.9036 60.3414 12.3841 60.3162 11.8718C63.017 10.1686 65.348 8.05987 67.2 5.64448C64.6771 6.61353 62.0036 7.2522 59.2662 7.53976C62.1496 6.03831 64.3092 3.6759 65.3436 0.891636C62.6345 2.28977 59.6695 3.27125 56.5782 3.7931C54.5 1.86498 51.7499 0.587651 48.754 0.158979C45.758 -0.269692 42.6834 0.174215 40.0064 1.42194C37.3294 2.66966 35.1994 4.65157 33.9463 7.06067C32.6932 9.46978 32.3869 12.1716 33.075 14.7477C27.5929 14.5082 22.2298 13.2676 17.3336 11.1062C12.4373 8.94483 8.11727 5.911 4.6536 2.2015C2.89523 4.84716 2.35861 7.97663 3.15271 10.9544C3.94682 13.9323 6.01211 16.5352 8.9292 18.2346C6.74327 18.1699 4.60539 17.6585 2.688 16.7418V16.9064C2.69177 19.6779 3.79389 22.3633 5.80807 24.5086C7.82225 26.6539 10.6249 28.1276 13.7424 28.6806C12.5591 28.9646 11.3368 29.1061 10.1094 29.1014C9.24413 29.1036 8.3806 29.0338 7.5306 28.8928C8.4117 31.2786 10.1273 33.3645 12.4372 34.8585C14.7471 36.3524 17.5356 37.1795 20.412 37.224C15.5255 40.5675 9.48948 42.3808 3.276 42.372C2.18115 42.376 1.08706 42.321 0 42.2074C6.30636 45.7098 13.6302 47.5696 21.1092 47.5676Z" fill="#7F7F7F"/>
                            </svg>
                        </a>

                    </div>
                </div>
                
            </div>

        </div>

        <p class='text-left mt-5 fw-normal'>© {{ date("Y") }}, Trisolace Company Ltd. {{ __('All rights reserved') }}.</p>

    </div>
</footer>